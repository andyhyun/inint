from token_type import TokenType
import error
import ast_class as ast


class Parser:

    def __init__(self, token_list):
        self.token_list = token_list
        self.tok_position = 0
        self.current_token = self.token_list[self.tok_position]

    def error(self, message=""):
        if len(message) == 0:
            raise error.ParserError(f"Unexpected {self.current_token.value} on line {self.current_token.line_number}")
        raise error.ParserError(message)

    def advance(self):
        self.tok_position += 1
        if self.tok_position >= len(self.token_list):
            self.current_token = None
        else:
            self.current_token = self.token_list[self.tok_position]

    def previous(self):
        return self.token_list[self.tok_position - 1]

    def check(self, tok_type):
        if self.current_token is None:
            return False
        return self.current_token.token_type == tok_type

    def match(self, *tok_types):
        for tok_type in tok_types:
            if self.check(tok_type):
                self.advance()
                return True
        return False

    def consume(self, tok_type, message=""):
        if self.check(tok_type):
            self.advance()
        else:
            self.error(message)

    def program(self):
        self.consume(TokenType.BEGIN, "Expected 'begin' at the beginning of the program")
        print("saw begin")  # DEBUG
        program_node = ast.Program(self.stmt_list())
        print("after last statement")  # DEBUG
        self.consume(TokenType.END, "Expected 'end' at the end of the program")
        print("saw end")  # DEBUG
        self.consume(TokenType.EOF, f"Unexpected {self.current_token.value} "
                                    f"after 'end' (Line {self.current_token.line_number})")
        print("at eof")  # DEBUG
        return program_node

    def stmt_list(self):
        print("in stmtlist")  # DEBUG
        stmt_list_node = self.stmt()
        is_end = False
        if stmt_list_node == -1:
            self.error("There are no statements in the program")
        print("saw statement")  # DEBUG
        while self.match(TokenType.SCOLON):
            right = self.stmt()
            if right == -1:
                is_end = True
                break
            print("saw statement")  # DEBUG
            stmt_list_node = ast.Chain(stmt_list_node, right)
        if is_end:
            print("returning list of stmts")  # DEBUG
            return stmt_list_node
        self.error(f"Expected ';' after statement (Line {self.current_token.line_number})")

    def stmt(self):
        if self.check(TokenType.PRINT):
            self.advance()
            print("about to start print stmt")  # DEBUG
            return self.print_stmt()
        elif self.check(TokenType.IDENT):
            print("about to start assign stmt")  # DEBUG
            self.advance()
            return self.assign_stmt()
        elif self.check(TokenType.IF):
            print("about to start if stmt")  # DEBUG
            self.advance()
            return self.if_stmt()
        elif self.check(TokenType.END):
            print("at end keyword at start of stmt")  # DEBUG
            return -1
        elif self.check(TokenType.EOF):
            self.error("Expected 'end' at the end of the program")
        self.error(f"Invalid statement at line {self.current_token.line_number}")

    def print_stmt(self):
        self.consume(TokenType.LPAREN, f"Expected '(' after 'print' (Line {self.current_token.line_number})")
        print("saw lparen for print")  # DEBUG
        expression = self.expr()
        print("have expr for print")  # DEBUG
        self.consume(TokenType.RPAREN, f"Expected ')' after the expression (Line {self.current_token.line_number})")
        print("saw rparen for print")  # DEBUG
        return ast.Print(expression)

    def assign_stmt(self):
        var = ast.Var(self.current_token.value)
        self.consume(TokenType.EQ, f"Expected '=' after the variable name (Line {self.current_token.line_number})")
        print("saw equals sign for assign stmt")  # DEBUG
        expression = self.expr()
        print("have expr for assign stmt")  # DEBUG
        return ast.Assign(var, expression)

    def if_stmt(self):
        self.consume(TokenType.LPAREN, f"Expected '(' after 'if' (Line {self.current_token.line_number})")
        print("saw lparen after if")  # DEBUG
        condition = self.expr()
        print("have expr for if stmt")  # DEBUG
        self.consume(TokenType.RPAREN, f"Expected ')' after the expression (Line {self.current_token.line_number})")
        print("saw rparen for if stmt after the expr")  # DEBUG
        self.consume(TokenType.THEN, f"Expected 'then' after ')' (Line {self.current_token.line_number})")
        print("saw then keyword after rparen for if stmt")  # DEBUG
        then_branch = self.stmt()
        print("have the then branch for if stmt")  # DEBUG
        return ast.If(condition, then_branch)

    def expr(self):
        expr_node = self.term()
        print("have term for expr")  # DEBUG
        while self.match(TokenType.PLUS, TokenType.MINUS):
            operator = self.previous()
            print("have operator for expr")  # DEBUG
            right = self.term()
            print("have term for expr")  # DEBUG
            expr_node = ast.Binary(expr_node, operator, right)
        print("returning expr node")  # DEBUG
        return expr_node

    def term(self):
        term_node = self.factor()
        print("have factor for term")  # DEBUG
        while self.match(TokenType.MULT, TokenType.DIV):
            operator = self.previous()
            print("have operator for term")  # DEBUG
            right = self.factor()
            print("have factor for term")  # DEBUG
            term_node = ast.Binary(term_node, operator, right)
        print("returning term node")  # DEBUG
        return term_node

    def factor(self):
        if self.check(TokenType.IDENT):
            self.advance()
            print("saw variable as factor")  # DEBUG
            return ast.Var(self.current_token.value)
        elif self.match(TokenType.ICONST, TokenType.RCONST, TokenType.SCONST):
            print("saw either int, real, or string literal as factor")  # DEBUG
            return ast.Literal(self.current_token.value)
        elif self.check(TokenType.LPAREN):
            print("saw lparen, beginning of parenthesized expr")  # DEBUG
            self.advance()
            expression = self.expr()
            print("have expr after lparen for factor")  # DEBUG
            self.consume(TokenType.RPAREN, f"Expected ')' after the expression (Line {self.current_token.line_number})")
            print("saw rparen after the expr, we have complete parenthesized expr")  # DEBUG
            return ast.Grouping(expression)
        else:
            self.error(f"Invalid character sequence (Line {self.current_token.line_number})")
